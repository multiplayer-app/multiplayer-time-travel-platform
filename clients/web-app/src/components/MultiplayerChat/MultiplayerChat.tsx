...
const MultiplayerChat = ({ character, preselectedQuestion, setQuestion, onDebuggerOpen }) => {
  ...
  const onHandleCharacterErrorResponse = useCallback(
    async (errorMessage: string, character: Character, isRetry: boolean) => {
      ...
      try {
        ...
        const response = await sendMessage(prompt, contextId, null, controller.signal)
        const data = response?.data

        const botMessage = createCharacterErrorMessage(data?.reply)
        addMessageWithDelay(botMessage, delay)
      } catch (err) {
        const fallbackMessage = createCharacterErrorMessage(character.errorMessage)
  ...
  const handleSuccess = (reply: string, newContextId: string, delay: number) => {
  ...
  const handleError = useCallback(
    async (error: any, delay: number, isRetry?: boolean) => {
      ...
      await onHandleCharacterErrorResponse(error.response?.data?.message || error.message, character, isRetry)
    },
  ...
  const postMessage = useCallback(
    async (message: string, isRetry?: boolean) => {
      ...
      try {
        ...
        const response = await sendMessage(message, contextId, character, controller.signal)
  ...
  const onCopy = useCallback(
    (message: string, index: number) => {
      try {
        ;(navigator as any).clipboard.writeMessage(message)
        ...
        showError('Failed to copy message. We are looking into it.')
      }
    },
    [setCopiedIndex]
  )
    ...
    async (msg: any, index: number) => {
      try {
        const prevUserMessage =
          msg.content.direction === 'outgoing'
            ? msg.message
        ...
        SessionRecorder.captureException(error)
        showError('Failed to retry message. We are looking into it.')
      }
    },
  ...
  return (
    <div className='mtt-chat-container'>
          ...
          <MessageList typingIndicator={isTyping && <TypingIndicator content={`${character.name} is typing...`} />}>
            {messages.map((msg, i) => (
              <div className={`mtt-message-row mtt-${msg.direction}`} key={i}>
                ...
                <div className={`cs-message__content ${msg.systemError ? 'cs-system-error' : ''}`}>
                  {msg.systemError && <div className='cs-system-error__placeholder'>Error</div>}
                  <Message.HtmlContent html={msg.message} />
                  {msg.systemError && (
                    <Message.CustomContent>
                      {recordingState !== SessionState.started && recordingState !== SessionState.paused ? (
                        <div className='mtt-debugger-toggle' onClick={onDebuggerOpen}>
                          👀 👈 Click “Record a new session” and let’s hunt another bug
                        </div>
                      ) : (
                        <div className='mtt-debugger-toggle' onClick={onSessionStopAndOpen}>
                ...
                <div className='mtt-message-actions'>
                  ...
                  />
                  <img src={retryIcon} alt='try again' title='Try again' onClick={() => onRetry(msg, i)} />
                </div>
              </div>
            ))}
            <div ref={bottomRef} />
      ...
      {preselectedQuestion && !isTyping && (
        <SuggestionsList character={character} onSelect={(msg) => postMessage(msg)} />
      )}
    </div>
  )
}
