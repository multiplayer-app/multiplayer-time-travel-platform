...
@RestController
@RequestMapping("/openrouter/messages")
public class OpenRouterController {
  ...
  @PostMapping
  @Operation(summary = "Send a message to OpenRouter", description = "Sends a message to the OpenRouter API and returns the assistant's response.", requestBody = @RequestBody(required = true, content = @Content(schema = @Schema(implementation = MessageRequest.class))), responses = {
  ...
  public ResponseEntity<?> sendMessageToOpenRouter(@RequestBody MessageRequest request) {
    ...
    try {
      ...
      // redisContextService.saveContext(contextId, updatedContext);
